...
export function LifecycleSettings(): React.JSX.Element {
  ...
  return (
    <section className={styles.card} aria-labelledby="lifecycle-heading">
      <header className={styles.cardHeader}><span className={styles.icon}>◫</span><div><small>本地服务</small><h2 id="lifecycle-heading">前端进程</h2><p>页面通过租约心跳复用同一个本地监听；最后一个页面离开后会自动回收。</p></div><span className={styles.badge}>{lifecycle.activeClients} 个页面</span></header>
