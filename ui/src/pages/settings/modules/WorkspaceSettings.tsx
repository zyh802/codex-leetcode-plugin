...
export function WorkspaceSettings(): React.JSX.Element {
  ...
  return (
    <section id="workspace-settings" tabIndex={-1} className={styles.card} aria-labelledby="directory-heading">
      <header className={styles.cardHeader}><span className={styles.icon}>▱</span><div><small>本地文件</small><h2 id="directory-heading">解答目录</h2><p>{workspace.settings.workspaceRoot ? `当前工作区：${workspace.settings.workspaceRoot}` : "未检测到当前工作区，请设置绝对路径"}</p></div></header>
